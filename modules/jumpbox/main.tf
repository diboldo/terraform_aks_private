...
resource "azurerm_linux_virtual_machine" "jumpbox" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      #"curl -o vsts-agent-linux-x64-2.210.1.tar.gz \ -L https://vstsagentpackage.azureedge.net/agent/2.210.1/vsts-agent-linux-x64-2.210.1.tar.gz"
      #"tar zxvf ./vsts-agent-linux-x64-2.194.0.tar.gz"
      #"./config.sh --unattended  --url "https://definedcrowd.visualstudio.com/DefinedCrowd" --auth "PAT" --token "vsts-agent@definedcrowd.com" --pool "AKS-Agent-Self-Hosted""
    ]
  }
}
